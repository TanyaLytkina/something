...
public class CarService {
    ...
    public List<Car> getNewCars(List<Car> cars) {
        return cars
                .stream()
                .filter(car -> car.getConditions.startsWith(NEW))
                .toList();
    }
    ...
    public Car getOldestCar(List<Car> cars) {
        return cars
                .stream()
                .max(Comporator.comparing(Car::getAge))
    ...
    public List<Car> getTwoBrokenCar(List<Car> cars) {
        return cars
                .stream()
                .filter(car -> car.getCondition.startsWith(BROKEN))
                .limit(2)
                .toList();
    }
    ...
    public List<Car> getSortedCarsByAge(List<Car> cars) {
        ...
        return cars
                .stream()
                .sorted(Comporator.comparing(e -> e.getAge))
                .toList();
    }
    ...
    public Boolean checkBrokenCarsAge(List<Car> cars) {
        return cars
                .stream()
                .allMatch(car -> car.getAge(car.getCondition.startsWith(BROKEN)) > 10);
    }
    ...
    public Boolean checkCarOwnerName(List<Car> cars) {
        return cars
                .stream()
                .anyMatch(car -> car.getName(car.getCondition.startsWith(USED)).equals("Adam"));
    }
    ...
    public Owner getAnyOwner(List<Car> cars) {
        return cars
                .stream()
                .map(car -> car.getOwners().stream())
                .findAny(owner -> owner.getAge() > 36);
    }
}
